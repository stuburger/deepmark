...
function ExemplarRow({ exemplar }: { exemplar: ExemplarAnswerListItem }) {
	...
	return (
		<TableRow>
			...
			<TableCell className="whitespace-nowrap">{exemplar.source_exam_board}</TableCell>
			<TableCell>
			...
			<TableCell className="overflow-hidden" title={exemplar.answer_text}>
				<p className="truncate text-sm text-muted-foreground">{truncate(exemplar.answer_text)}</p>
			</TableCell>
		...
		<div className="space-y-6">
			...
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Exemplar answers linked to questions and mark schemes from ingested PDFs.
					</CardDescription>
				</CardHeader>
				<CardContent>
					{!result.ok ? (
