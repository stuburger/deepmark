	...
	const upload = useCallback(
		async (file: File) => {
			...
			try {
				...
				setProgress(
					`Creating upload for ${blobs.length} page${
						blobs.length !== 1 ? "s" : ""
					}…`,
				)
				...
				for (let i = 0; i < result.presignedPutUrls.length; i++) {
					...
					if (!entry) continue
					setProgress(
						`Uploading page ${entry.pageNumber} of ${result.presignedPutUrls.length}…`,
					)
					const blob = blobs[i]
					if (!blob) continue
		...
		<main className="mx-auto flex min-h-screen w-full max-w-2xl flex-col gap-6 px-6 py-16">
			<Card>
				<CardHeader>
					...
					<CardDescription>
						Upload a JPEG, PNG, WebP image or a multi-page PDF. PDFs are
						converted to images in your browser before uploading. OCR runs
						automatically.
					</CardDescription>
				</CardHeader>
