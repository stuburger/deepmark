...
import {
  getActiveIngestionJobsForExamPaper,
...
export function ExamPaperPageShell({ paper }: { paper: ExamPaperDetail }) {
  ...
  const fetchDocs = useCallback(async () => {
    const r = await getPdfDocumentsForPaper(paper.id);
    if (r.ok) setCompletedDocs(r.documents);
  }, [paper.id]);
