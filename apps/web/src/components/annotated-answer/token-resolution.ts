...
/**
 * Resolve a PM range `[from, to)` to a single token-range handle:
 * `{ questionId, tokenStart, tokenEnd }`, where `tokenStart` and `tokenEnd`
 * are the IDs of the first and last tokens (in spatial order) overlapping
 * the range inside a single `questionAnswer` block.
 *
 * Returns null when:
 *  - the range is collapsed (from === to)
 *  - the range overlaps zero or more than one question block
 *  - the alignment for the overlapping question isn't loaded
 *  - the range maps to char positions but no tokens overlap (e.g. inter-
 *    word whitespace)
 *
 * Used by the editor's "Talk to DeepMark" bubble trigger to attach
 * machine-referenceable token IDs to the selection chip — the model can
 * call `addAnnotation` against the exact range the teacher highlighted.
 */
export function resolveTokenRangeForSelection(
	doc: PmNode,
	from: number,
	to: number,
	alignmentByQuestion: ReadonlyMap<string, TokenAlignment>,
): { questionId: string; tokenStart: string; tokenEnd: string } | null {
	...
	const [{ questionId, block, blockStart, overlapFrom, overlapTo }] = overlaps

	const alignment = alignmentByQuestion.get(questionId)
	if (!alignment) return null

	const charFrom = pmPosToCharInBlock(block, blockStart, overlapFrom)
	const charTo = pmPosToCharInBlock(block, blockStart, overlapTo)
	if (charFrom === null || charTo === null) return null

	const clampedTo = Math.max(charTo, charFrom + 1)
	const ids = tokenIdsInRange(charFrom, clampedTo, alignment)
	if (ids.length === 0) return null

	// tokenIdsInRange returns in tokenMap insertion order, which is normally
	// spatial — but per its contract callers must sort by range.start for
	// strict spatial order. Cheap O(n log n) for the usually-small hit set.
	const sorted = [...ids].sort(
		(a, b) => alignment.tokenMap[a].start - alignment.tokenMap[b].start,
	)
	return {
		questionId,
		tokenStart: sorted[0],
		tokenEnd: sorted[sorted.length - 1],
	}
}
...
export function resolveTokenAtCursor(
	...
	if (!alignment) return null
	return (
		tokenIdAtChar(pt.char, alignment) ?? tokenIdAtChar(pt.char - 1, alignment)
	)
}
