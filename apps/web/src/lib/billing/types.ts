 ...
 * monthly + annual; Unlimited is monthly-only (we don't surface annual).
 */
export type PlanKind = "pro" | "unlimited"
 ...
 */
export type PlanId = "pro_monthly" | "pro_annual" | "unlimited_monthly"
 ...
 * copy: trial → buy/subscribe; capped Pro → top up / Unlimited;
...
function defaultInsufficientBalanceMessage(
	...
	if (plan === "pro_monthly") {
		return `Monthly limit hit — top up for £6.50 (15 papers) or upgrade to Unlimited. Need ${short} more.`
	}
	...
	// unlimited_monthly should never hit this branch (it resolves to
