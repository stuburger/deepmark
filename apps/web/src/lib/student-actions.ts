...
export type ConfirmStudentResult = { ok: true } | { ok: false; error: string }

export async function confirmStudentForSubmission(
	submissionId: string,
	studentId: string,
): Promise<ConfirmStudentResult> {
	const session = await auth()
	if (!session) return { ok: false, error: "Not authenticated" }
...
export type CreateAndConfirmResult =
	| { ok: true; student: StudentItem }
	| { ok: false; error: string }

export async function createAndConfirmStudent(
	submissionId: string,
	name: string,
	className?: string,
