 ...
 *   - Per-method shape: point_based ⇒ mark_points present;
 *     level_of_response ⇒ levels present; deterministic ⇒ correct_option set.
 *
...
export function validateBundle(bundle: PaperBundle): BundleValidation {
	...
	for (const section of bundle.sections) {
		...
		for (const q of section.questions) {
			...
			if (
				method === "level_of_response" &&
				(!q.mark_scheme.levels || q.mark_scheme.levels.length === 0)
			) {
				return {
					ok: false,
					error: `Level-of-response question "${label}" has no levels`,
				}
			}
