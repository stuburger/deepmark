...
export const handler = tool(
	...
	async (args, extra) => {
		...
		while (refinementCycle <= max_refinement_cycles) {
			console.log(
				`[test-and-refine-mark-scheme] Running test cycle ${refinementCycle}`,
			)
			...
			console.log(
				`[test-and-refine-mark-scheme] Test results: ${testSummary.overall_accuracy}% accuracy`,
			)
			...
			console.log(
				`[test-and-refine-mark-scheme] Refining mark scheme (cycle ${refinementCycle + 1})`,
			)
		...
		if (shouldSave) {
			...
			console.log(
				"[test-and-refine-mark-scheme] Mark scheme updated successfully",
			)
		}
