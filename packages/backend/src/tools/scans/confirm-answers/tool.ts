...
export const handler = tool(ConfirmScanAnswersSchema, async (args, extra) => {
	...
	if (!submission.student_id) {
		throw new Error(
			"A student must be confirmed for this submission before answers can be saved. Use confirm-student first.",
		)
	}
	...
	const extracted = await db.extractedAnswer.findMany({
		where: { scan_submission_id, answer_id: null },
		include: {
			question: true,
			question_part: true,
		},
	...
	for (const e of extracted) {
		...
		if (
			...
			result.max_possible_score != null
		) {
			markingResults.push({
				answer_id: answer.id,
	...
	return JSON.stringify(
		{
			...
			max_possible: markingResults.reduce(
				(s, r) => s + r.max_possible_score,
				0,
			),
		},
		null,
