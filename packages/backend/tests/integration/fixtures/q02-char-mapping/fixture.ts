 ...
 *
 * Same source data as the web regression fixture at
 * `apps/web/src/lib/marking/alignment/__tests__/fixtures/q02-subhaan-baig.ts`.
 * Duplicated here because tests cannot import across package boundaries.
 *
 * Tokens are in Cloud Vision's native `(page, para, line, word)` order. For
 * the Option C eval we re-sort them into spatial reading order (matching
 * how the attribution LLM authored `Q02_ANSWER_TEXT`) before feeding the
 * mapper.
 */

export type Q02FixtureToken = {
	id: string
	page_order: number
	text_raw: string
...
export const Q02_TOKENS: Q02FixtureToken[] = [
