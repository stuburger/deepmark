...
describe.each(AVAILABLE_FIXTURES)(
	"pdf segmentation evals — $name",
	(fixture) => {
		...
		it(`returns a plausible number of scripts (within ${fixture.thresholds.scriptCountTolerance} of ground truth)`, () => {
			...
			expect(segmented.length).toBeGreaterThanOrEqual(
				fixture.scripts.length - tol,
			)
...
async function extractPages(pdfBytes: Uint8Array): Promise<SegmentPageInput[]> {
